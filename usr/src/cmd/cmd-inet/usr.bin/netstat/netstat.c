 ...
 *
 * NOTES:
 * 1. A comment "LINTED E_BAD_PTR_CAST_ALIGN" appears before certain lines
 *    where lint would have complained, "pointer cast may result in improper
...
tcp_report(const mib_item_t *item)
{
	...
	for (; item != NULL; item = item->next_item) {
		...
		if (!((!Uflag && item->group == MIB2_TCP &&
		    ...
		    (!Uflag && item->group == MIB2_TCP6 &&
		    item->mib_id == MIB2_TCP6_CONN) ||
		    (Uflag && item->group == MIB2_TCP &&
		...
		if (item->group == MIB2_TCP) {
			for (tp = (mib2_tcpConnEntry_t *)item->valp;
			    ...
			    tcpConnEntrySize +
			    (cpi && item->mib_id == EXPER_XPORT_PROC_INFO ?
			    cpi->cpi_tot_size : 0))) {
				aptr = v4a == NULL ? NULL : *v4a++;
				cpi = item->mib_id == EXPER_XPORT_PROC_INFO ?
				    /* LINTED E_BAD_PTR_CAST_ALIGN */
				    (conn_pid_info_t *)((char *)tp +
				    tcpConnEntrySize) : NULL;
			...
			for (tp6 = (mib2_tcp6ConnEntry_t *)item->valp;
				...
				cpi = item->mib_id == EXPER_XPORT_PROC_INFO ?
				    /* LINTED E_BAD_PTR_CAST_ALIGN */
				    (conn_pid_info_t *)((char *)tp6 +
				    tcp6ConnEntrySize) : NULL;
...
udp_report(const mib_item_t *item)
{
	...
	for (; item; item = item->next_item) {
		...
		if (!((!Uflag && item->group == MIB2_UDP &&
		    ...
		    (!Uflag && item->group == MIB2_UDP6 &&
		    item->mib_id == MIB2_UDP6_ENTRY) ||
		    (Uflag && item->group == MIB2_UDP &&
		    item->mib_id == EXPER_XPORT_PROC_INFO) ||
		    (Uflag && item->group == MIB2_UDP6 &&
		    item->mib_id == EXPER_XPORT_PROC_INFO)))
			continue; /* 'for' loop 1 */
    ...
    boolean_t first, const mib2_transportMLPEntry_t *attr)
{
	...
	if (!Uflag || cpi == NULL) {
		...
		do {
			pinfo = get_proc_info(*pids);
			(void) printf(UDP_V4_USER_F " " UDP_V4_PID_F " ",
    ...
    boolean_t first, const mib2_transportMLPEntry_t *attr)
{
	...
	(void) printf(UDP_V6_LOCAL_F " " UDP_V6_REMOTE_F " ",
	...
	if (!Uflag || cpi == NULL) {
		...
		do {
			pinfo = get_proc_info(*pids);
			(void) printf(UDP_V6_USER_F " " UDP_V6_PID_F " ",
			    pinfo->pr_user, (int)*pids);
			if (Vflag) {
				(void) printf(
...
sctp_report(const mib_item_t *item)
{
	...
	for (; item != NULL; item = item->next_item) {
		...
		if (!Uflag && item->mib_id != MIB2_SCTP_CONN)
			continue;
		if (Uflag && item->mib_id != EXPER_XPORT_PROC_INFO)
			continue;
		...
		for (sp = item->valp;
			...
			cpi = item->mib_id == EXPER_XPORT_PROC_INFO ?
			    /* LINTED E_BAD_PTR_CAST_ALIGN */
			    (conn_pid_info_t *)((char *)sp + sctpEntrySize) :
			    NULL;
...
get_proc_info(pid_t pid)
{
	...
	static proc_info_t unknown_proc_info = {
	    .pr_user = "<unknown>",
	    .pr_fname = "",
	    .pr_psargs = ""
	};
	static proc_info_t sched_proc_info = {
	    .pr_user = "root",
	    .pr_fname = "sched",
	    .pr_psargs = "sched"
	};
	...
	/* hardcode pid = 0 */
	if (pid == 0)
		return (&sched_proc_info);
	if (pid == saved_pid)
		return (&saved_proc_info);
	if (snprintf(path, sizeof (path), "/proc/%u/psinfo", (int)pid) <= 0)
...
static const char uds_hdr_pid[] =
    UDS_TYPE " " UDS_USER " " UDS_PID " " UDS_COMMAND " "
    UDS_LOCAL " " UDS_REMOTE "\n"
    UDS_TYPE_" " UDS_USER_" " UDS_PID_" " UDS_COMMAND_" "
    UDS_LOCAL_" " UDS_REMOTE_"\n";

static const char uds_hdr_pid_verbose[] =
    UDS_TYPE " " UDS_USER " " UDS_PID " "
    UDS_LOCAL " " UDS_REMOTE " " UDS_COMMAND "\n"
    UDS_TYPE_" " UDS_USER_" " UDS_PID_" "
