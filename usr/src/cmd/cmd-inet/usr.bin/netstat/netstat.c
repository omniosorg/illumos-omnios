...
void fail(int, char *, ...)  __attribute__((__noreturn__));
...
typedef struct proc_hash {
	...
	struct proc_hash *ph_next;	/* Next (for collisions) */
	struct proc_hash *ph_next_proc;	/* Next process with this inode */
} proc_hash_t;

static proc_hash_t *process_table[PROC_HASH_SIZE];
...
static proc_hash_t *
process_hash_get(const mib2_socketInfoEntry_t *sie)
{
	proc_hash_t *ph;
	uint_t idx = PROC_HASH(sie->sie_inode);

	static proc_hash_t unknown_proc = {
		.ph_pid = 0,
		.ph_pidstr = "",
		.ph_username = "",
		.ph_fname = "",
	...
	if (Xflag) {
		(void) snprintf(unknown_proc.ph_pidstr, PROC_PID_SIZE,
		    "INODE %" PRIu64, sie->sie_inode);
	}
...
static void
process_hash_insert(proc_hash_t *ph)
{
	uint_t idx = PROC_HASH(ph->ph_inode);
	proc_hash_t *slotp;

	for (slotp = process_table[idx]; slotp != NULL;
	    slotp = slotp->ph_next) {
		if (slotp->ph_inode == ph->ph_inode &&
		    (slotp->ph_mode & S_IFMT) == (ph->ph_mode & S_IFMT) &&
		    slotp->ph_major == ph->ph_major)
			break;
	}
...
static int
process_hash_iter(void *data, prfdinfo_t *pr)
{
	psinfo_t *psinfo = (psinfo_t *)data;
	proc_hash_t *ph;

	/*
	 * We are interested in sockets and fds linked to
	 * network STREAMS devices.
	 */
	...
	if ((ph = calloc(1, sizeof (proc_hash_t))) == NULL)
		return (0);
...
process_hash_iterate(psinfo_t *psinfo)
{
	...
	while ((ent = readdir(dirp)) != NULL) {
		...
		if (process_hash_iter((void *)psinfo, &info) != 0)
			break;
	}
	(void) closedir(dirp);
}
...
process_hash_build()
{
	...
	while ((proce = readdir(proc)) != NULL) {
		...
		if ((ph = Pgrab(pid, PGRAB_RETAIN | PGRAB_NOSTOP,
		    &err)) == NULL) {
		...
		(void) Pfdinfo_iter(ph, process_hash_iter, &psinfo);
...
process_hash_free()
{
	...
	for (i = 0; i < PROC_HASH_SIZE; i++) {
		for (ph = process_table[i]; ph != NULL; ph = ph2) {
			ph2 = ph->ph_next;
			free(ph);
		}
...
mib_item_diff(mib_item_t *item1, mib_item_t *item2)
{
	...
	for (tempp2 = item2; tempp2 != NULL; tempp2 = tempp2->next_item) {
		...
		for (tempp1 = item1; tempp1 != NULL;
			...
			case MIB2_IP: {
				...
				d = (mib2_ip_t *)calloc(1, tempp2->length);
				if (d == NULL)
			...
			case EXPER_DVMRP: {
				...
				diffptr->mib_id = tempp2->mib_id;
				diffptr->length = tempp2->length;
				d = (struct mrtstat *)calloc(1, tempp2->length);
				if (d == NULL)
					goto mibdiff_out_of_memory;
				diffptr->valp = d;
			...
			case MIB2_ICMP: {
				...
				diffptr->mib_id = tempp2->mib_id;
				diffptr->length = tempp2->length;
				d = (mib2_icmp_t *)calloc(1, tempp2->length);
				if (d == NULL)
					goto mibdiff_out_of_memory;
				diffptr->valp = d;
	...
	diffptr->mib_id = tempp2->mib_id;
	diffptr->length = tempp2->length;
	d = (mib2_ipv6IfIcmpEntry_t *)calloc(1, tempp2->length);
	if (d == NULL)
		goto mibdiff_out_of_memory;
	diffptr->valp = d;
	...
	break;
			...
			case MIB2_TCP: {
				...
				diffptr->mib_id = tempp2->mib_id;
				diffptr->length = tempp2->length;
				d = (mib2_tcp_t *)calloc(1, tempp2->length);
				if (d == NULL)
					goto mibdiff_out_of_memory;
				diffptr->valp = d;
			...
			case MIB2_UDP: {
				...
				diffptr->mib_id = tempp2->mib_id;
				diffptr->length = tempp2->length;
				d = (mib2_udp_t *)calloc(1, tempp2->length);
				if (d == NULL)
					goto mibdiff_out_of_memory;
				diffptr->valp = d;
			...
			case MIB2_SCTP: {
				...
				diffptr->mib_id = tempp2->mib_id;
				diffptr->length = tempp2->length;
				d = (mib2_sctp_t *)calloc(1, tempp2->length);
				if (d == NULL)
					goto mibdiff_out_of_memory;
				diffptr->valp = d;
			...
			case EXPER_RAWIP: {
				...
				diffptr->mib_id = tempp2->mib_id;
				diffptr->length = tempp2->length;
				d = (mib2_rawip_t *)calloc(1, tempp2->length);
				if (d == NULL)
					goto mibdiff_out_of_memory;
				diffptr->valp = d;
...
gather_info(const mib_item_t *item, int group, int mib_id, int esize)
{
	...
	for (iptr = item; iptr != NULL; iptr = iptr->next_item) {
		...
		for (sie = (mib2_socketInfoEntry_t *)iptr->valp;
		    (char *)sie < (char *)iptr->valp + iptr->length; sie++) {
			if (Xflag) {
    ...
    const mib2_transportMLPEntry_t *attr, const mib2_socketInfoEntry_t *sie)
{
	...
	if (!Uflag && Vflag) {
		...
		for (ph = process_hash_get(sie); ph != NULL;
		    ph = ph->ph_next_proc) {
			(void) printf(
			    TCP_V4_LOCAL_F "\n" TCP_V4_REMOTE_F " "
			    TCP_V4_SWIND_F " " TCP_V4_SNEXT_F " "
    ...
    const mib2_transportMLPEntry_t *attr, const mib2_socketInfoEntry_t *sie)
{
	...
	(void) snprintf(leadin, sizeof (leadin),
...
uds_report_item(struct sockinfo *psi, boolean_t first)
{
	...
	for (ph = process_hash_get(&sie); ph != NULL; ph = ph->ph_next_proc) {
		if (Vflag) {
			(void) printf(
