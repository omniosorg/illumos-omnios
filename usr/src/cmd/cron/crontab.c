...
copycron(FILE *fp)
{
	...
	while (fgets(line, CTLINESIZE, fp) != NULL) {
		...
		if (next_field(0, 59, line, &cursor, &cferr) == NULL ||
		    next_field(0, 23, line, &cursor, &cferr) == NULL ||
		    next_field(1, 31, line, &cursor, &cferr) == NULL ||
		    next_field(1, 12, line, &cursor, &cferr) == NULL ||
		    next_field(0, 6, line, &cursor, &cferr) == NULL) {
			switch (cferr) {
			case CFIELD_EOLN:
				cerror(EOLN);
				break;
			case CFIELD_UNEXPECT:
				cerror(UNEXPECT);
				break;
			case CFIELD_OUTOFBOUND:
				cerror(OUTOFBOUND);
				break;
			case CFIELD_EOVERFLOW:
				cerror(OVERFLOW);
				break;
			case CFIELD_ENOMEM:
