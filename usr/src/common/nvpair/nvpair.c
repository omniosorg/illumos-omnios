 ...
 * the '\0' terminator byte when calculating string size.  If 'max_size' is 0,
 * it is ignored.
 */
static int
    ...
    size_t max_size)
{
	...
	/*
	 * A max_size of 0 means that the caller is not imposing a limit.
	 * Since the value size is calculated as an int, and anything larger
	 * than INT32_MAX is rejected below, INT32_MAX is the effective
	 * limit in any case.
	 */
	if (max_size == 0)
		max_size = INT32_MAX;
...
nvpair_value_common(nvpair_t *nvp, data_type_t type, uint_t *nelem, void *data)
{
	...
	case DATA_TYPE_DOUBLE:
		...
		bcopy(NVP_VALUE(nvp), data,
		    (size_t)i_get_value_size(type, NULL, 1, 0));
		if (nelem != NULL)
			*nelem = 1;
		break;
...
nvpair_native_string_array(nvstream_t *nvs, nvpair_t *nvp)
{
	...
	case NVS_OP_DECODE: {
		...
		size_t max_size = NVP_SIZE(nvp) - (buf - (char *)nvp);
		size_t len;
		int i;

		/*
		 * The string data must lie entirely within this nvpair.
		 * Bound each length calculation so that a string which is
		 * not terminated cannot take the walk past the end of the
		 * buffer.
		 */
		for (i = 0; i < NVP_NELEM(nvp); i++) {
			len = strnlen(buf, max_size);
			if (len == max_size)
				return (EFAULT); /* not terminated */
			strp[i] = buf;
			buf += len + 1;
			max_size -= len + 1;
		}
