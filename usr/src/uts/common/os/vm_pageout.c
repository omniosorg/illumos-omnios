...
loop:
	...
	while (nscan_cnt < desscan) {
		 ...
		 *  1) during page scan startup when determining sample data
		 *  2) there is not enough free memory
		 *  3) one or more zones is over its cap
		 */
...
checkpage(page_t *pp, pageout_hand_t whichhand)
{
	...
	if (zones_over) {
		...
		if (pp->p_zoneid == ALL_ZONES ||
			...
			/*
			 * Cross-zone shared page, or zone not over its cap.
