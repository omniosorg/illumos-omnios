...
virtio_mmio_init(dev_info_t *dip)
{
	 ...
	 */
	if (virtio_mmio_get32(vio, VIRTIO_MMIO_MAGIC_VALUE) !=
	    VIRTIO_MMIO_MAGIC) {
		dev_err(dip, CE_WARN, "bad MMIO magic value");
		ddi_regs_map_free(&vio->vio_barh);
		kmem_free(vio, sizeof (*vio));
		return (NULL);
	}
	if (virtio_mmio_get32(vio, VIRTIO_MMIO_VERSION) != 1) {
		dev_err(dip, CE_WARN,
		    "unsupported MMIO version %u (expected 1)",
		    virtio_mmio_get32(vio, VIRTIO_MMIO_VERSION));
		ddi_regs_map_free(&vio->vio_barh);
		kmem_free(vio, sizeof (*vio));
		return (NULL);
	}
