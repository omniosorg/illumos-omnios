...
static bool
virtio_mmio_postinit(virtio_t *vio)
{
	uint32_t r;

	/*
	 * Verify the MMIO magic number and version.
	 */
	r = virtio_mmio_get32(vio, VIRTIO_MMIO_MAGIC_VALUE);
	if (r != VIRTIO_MMIO_MAGIC) {
		dev_err(vio->vio_dip, CE_WARN,
		    "bad MMIO magic value 0x%" PRIx32, r);
		return (false);
	}
	r = virtio_mmio_get32(vio, VIRTIO_MMIO_VERSION);
	if (r != 1) {
		dev_err(vio->vio_dip, CE_WARN,
		    "unsupported MMIO version 0x%x (expected 1)", r);
		return (false);
	}
...
virtio_ops_t virtio_mmio_ops = {
	.vop_postinit = virtio_mmio_postinit,
